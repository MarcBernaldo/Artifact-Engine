...
- `_utc` — value is derived from an epoch and rendered UTC (`fromtimestamp(sec,
  ...
  and `collection_artifacts.{first_created,last_created}_utc` (the earliest and
  latest crtime under a mirrored tree — on a collection tree, the acquisition).
...
Since v0.7.23 the convention finally has a CONSUMER: `core/findings.py` reads the
column out of the consolidated `.db` and puts the flagged rows into `report.txt`
and `findings.csv`. Until then nothing in `core/` read it at all — every parser
wrote the flag and the only way to see one was to open the database and write
SQL. Two consequences for a new parser: writing the column is now what puts a
finding in front of the analyst, and writing it too readily is now what buries
everyone else's (see the ranking below).
...
- **Windows filesystem**: mft_transcode ($MFT, MFTECmd), timestomp_mft (files
  ...
  collection_artifacts_mft (directories whose children are the volume's own root
  — a copy of the volume written onto it, which is what a collector pointed at
  the disk it is collecting produces; those paths are dropped from `aeng sweep`
  by default, `Windows.old` and a collector's tool directory are named in the
  same table and never are), usn ($J/UsnJrnl), lnk (LECmd), jumplists (JLECmd),
  recyclebin (RBCmd).
...
- **Linux logs / timelines**: auth (sshd/sudo/su/useradd from
  ...
  filesystem MAC timeline, streamed), collection_artifacts_bodyfile (the same
  shape test against `/`: a UAC output tree left on the host, an extracted
  acquisition, a chroot or a container rootfs), timestomp_bodyfile (`touch -r` copies
