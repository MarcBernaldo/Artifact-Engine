...
the shared `iter_utmp`, and read the same way: the current file plus every
rotation. Complements logins.csv (which carries the text `lastb` output) with
the always-present binary source: the `host` column is the source IP/host of
each attempt and `user` is the account that was tried.
"""
